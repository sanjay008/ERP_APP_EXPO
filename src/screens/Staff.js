...
const Staff = ({ navigation, route }) => {
  ...
  const requestCameraPermissionIOS = async () => {
    try {
      ...
      return result; // Will be one of RESULTS.GRANTED, RESULTS.DENIED, etc.
    } catch (error) {
      console.warn("Error requesting iOS camera permission: ", error);
      return RESULTS.DENIED; // Default to "denied" if there's an error
    }
  };
  ...
  //     } else if (Platform.OS === "ios") {
  //       // iOS-specific permission request
  //       // Use the iOS permission API (e.g., react-native-permissions)
  //       const permissionStatus = await requestCameraPermissionIOS(); // Implement this function for iOS
        ...
        <View style={styles.container}>
          ...
          {scomplate != true ? (
            ...
            <Text style={styles.buttonText}>Select Birth Date </Text>
                    ...
                    {/* Display selected final date outside the modal */}
                    {/* {finalDate && (
            <Text style={styles.selectedDateText}>
              Selected Date: {finalDate}
                    ...
                    {/* <View style={styles.dateInput}>
                      ...
                      >
                        {finalDate ? finalDate : "Select Date"}
                      </Text>
                      <TouchableOpacity
                        style={{
                      ...
                      <View style={styles.modalContainer}>
                        ...
                        {step === 1 && (
                          <View>
                            ...
                            <Text style={styles.title}>Select Decade</Text>
                                ...
                                return (
                                  <TouchableOpacity
                                    ...
                                    onPress={() => {
                                      ...
                                      setStep(2); // Automatically proceed to select year after selecting a decade
                                    }}
                                  >
                                    <Text
                                      style={{
                        ...
                        {step === 2 && selectedDecade && (
                          <View>
                            <Text style={styles.title}>Select Year</Text>
                            <View style={styles.row}>
                              {Array.from(
                                { length: 10 },
                                (_, i) => selectedDecade + i
                              ).map((year) => (
                                <TouchableOpacity
                                  key={year}
                                  style={[
                            ...
                            <View style={styles.row}>
                              {months.map((month, index) => (
                                <TouchableOpacity
                                  ...
                                  style={[
                                    styles.monthButton,
                                    selectedMonth === index && styles.selected,
                                  ]}
