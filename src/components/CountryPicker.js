...
const defaultStyles = StyleSheet.create({
  ...
  callingCodeText: { marginLeft: 4, fontSize: 16, color: "black" },
  phoneInput: {
    flex: 1,
  ...
  searchInput: { padding: 8, borderRadius: 8, borderWidth: 1, borderColor: "#ddd",color:"black" },
  itemRow: { flexDirection: "row", alignItems: "center", padding: 12, borderBottomWidth: 1, borderColor: "#fafafa", gap: 5 },
  CountyText: { color: Colors.black, fontSize: 14 }
});
...
export default function MyCountryPiker({
 ...
 useEffect(()=>{
    setSelected(
    countriesData.find((c) => (c?.cca2 === defaultCountry) || c?.countrycode == defaultCountry) || countriesData[0]
    )
  },[defaultCountry])
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity  style={defaultStyles.itemRow} onPress={() => onPick(item)}>
      ...
      {showCallingCode && <Text style={[defaultStyles.CountyText, { fontFamily: FontFamily }]}>+{item.countrycode}</Text>}
      <View style={{ marginLeft: 8, }}>
        <Text style={[defaultStyles.CountyText, { fontFamily: FontFamily }]}>{item.countryname}</Text>
      </View>
    </TouchableOpacity>
  );
    ...
    <View style={[defaultStyles.container,]}>
      <View style={[defaultStyles.inputContainer, ContainerStyle]}>
        <TouchableOpacity  disabled={disbled} style={defaultStyles.pickerButton} onPress={openModal}>
          ...
          {showCallingCode && <Text  style={[defaultStyles.callingCodeText, { fontFamily: FontFamily }, CountyCodeStyle]}>+{selected?.countrycode}</Text>}
        </TouchableOpacity>
        {showPhoneInput && (
          <TextInput
            key={defaultCountry}
            style={[defaultStyles.phoneInput, { fontFamily: FontFamily }, TextInputStyle]}
            placeholder="Phone number"
            keyboardType="numeric"
            editable={!disbled}
            value={value}
            maxLength={["31","91","597"]?.includes(selected?.countrycode) ? selected.phoneLength : 17}
      ...
      <Modal transparent visible={visible} animationType="none">
        ...
        <Animated.View
          style={{
            ...
            right: 0,
            bottom: 0,
            height: KeyboardVisible ? "50%" : modalHeight,
            // top: slideAnim,
