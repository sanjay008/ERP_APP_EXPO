...
export default function NewUserSignupScreen() {
  ...
  const ensureSession = useCallback(() => {
    if (!userId || !verifyToken) {
      router.replace("/registration/signup");
    }
  }, [router, userId, verifyToken]);
  ...
  const pickImage = async (source: "camera" | "gallery") => {
    ...
    if (result.reason === "error" || result.reason === "unavailable") {
  ...
  const handleSubmit = async () => {
    if (!validateStep3() || !userId || !verifyToken) return;

    setLoading(true);
    setToastMessage(null);

    try {
      const response = await createUserWithRelaties({
        verifyToken,
        userId,
        salutation,
...
const styles = StyleSheet.create({
  ...
  dateText: { flex: 1 },
  errorSpacing: { marginTop: 6 },
  errorCenter: { marginTop: 8 },
  photoWrap: { alignSelf: "center", marginBottom: 12, marginTop: 8 },
  photo: { width: 120, height: 120, borderRadius: 60, backgroundColor: "#EEF2F7" },
  cameraBadge: {
    position: "absolute",
    right: 0,
    bottom: 0,
  ...
  cameraIcon: { width: 18, height: 18, tintColor: AppColors.white },
  photoHint: { textAlign: "center", color: AppColors.subtitle, marginBottom: 8 },
  sheetOverlay: {
    flex: 1,
  ...
  sheetOption: { flexDirection: "row", alignItems: "center", gap: 12, paddingVertical: 12 },
  sheetOptionText: {
